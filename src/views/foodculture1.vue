<template>
    <div class="foodculture">
      ...
      <p class="foodculturep"><img src="../assets/img/lication.png" alt="">当前位置：<a href="/">首页</a>>食文化</p>
          ...
          <div class="foodculture-list" v-for="(item,index) in newsList" :key="index" @click="wacthFoodsxq(item)">
            ...
            <div class="foodculture-list-more">
              <router-link
                ...
                :to="{ name: 'foodcultureDetails1', params: { id: item.Id }}"
              >
...
export default {
  ...
  methods: {
    //食文化列表
      foodsCulturePageList(){
        this.$http.get('/cultureapi/foodCulturePageList?page='+this.page+"&size="+this.size).then(res =>{
      ...
      currentChangeHandle(val){
          this.page = val -1;
          this.foodsCulturePageList();
      },
      // 点击进入食文化详情页
     wacthFoodsxq(item){
      this.$router.push({path:'foodcultureDetails1',query:{uuid: item.cultureInfoUuid}})
    },
  },
  mounted() {
    this.foodsCulturePageList()
