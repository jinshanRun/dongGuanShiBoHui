<template>
    <div class="foodculture">
      ...
      <p class="foodculturep"><img src="../assets/img/lication.png" alt="">当前位置：<a href="/">首页</a>>食文化><span>茶文化</span></p>
          ...
          <div class="foodculture-list" v-for="(item,index) in newsList" :key="index" @click="wacthFoodsxq(item)">
            ...
            <div class="foodculture-list-more">
              <router-link
                ...
                :to="{ name: 'foodcultureDetails2', params: { id: item.Id }}"
              >
...
export default {
  ...
  methods: {
    //茶文化列表
      teaCulturePageList(){
          this.$http.get('/cultureapi/teaCulturePageList?page='+this.page+"&size="+this.size).then(res =>{
      ...
      currentChangeHandle(val){
          this.page = val -1;
          this.teaCulturePageList();
      },
      // 点击进入茶文化详情页
     wacthFoodsxq(item){
      this.$router.push({path:'foodcultureDetails2',query:{uuid: item.cultureInfoUuid}})
    },
  },
  mounted() {
    this.teaCulturePageList();
