<template>
    <div class="webexh">
      <div class="webexh-banner" v-loading="loading"><img src="../../assets/img/webexbck1.png" alt=""></div>
     <div class="webexhtxt">
        <p class="foodculturep">
          <img src="../../assets/img/lication.png" alt="">当前位置：<a href="/">首页</a>>网上展厅>线上展厅
        </p>
       <div class="h40"></div>
       ...
       <div class="webexhadv_right"><img src="../assets/img/newsadver.png" alt=""></div> -->
        ...
        <ul class="companylogoimg" >
          <li v-for="(item,i) in companyList" :key="i" @click="companyYnDetail(item.companyUuid)">
            <img :src=item.logoUrl alt="">
          </li>
        </ul>
        <div class="ZSlist">
          <p style="color: #333333;font-size:20px;font-weight:600"><span>| </span>展商列表</p>
          <ul v-for="(item,i) in normalCompanyList" :key="i">
            <li @click="companyYnDetail(item.companyUuid)">
              ...
              <div class="seenumber">
                <span>{{item.lookCount !== null ? item.lookCount : 0}}</span>
                <!-- <span>3.2k</span>
                <span>1.8k</span> -->
              </div>
            </li>
...
export default {
  ...
  data() {
    return {
      loading: true,
      normalCompanyList:null,
      companyList: null,
      uuid: this.$route.query.uuid !==undefined ? this.$route.query.uuid : 'ff7632e456174ad4b52d6fc76968f35420201023172642',
      page: 0,
      size: 5,
      // 总数
      countTotal: 0,
  ...
  methods: {
    ...
    hotcompanyList(uuid){
     ...
     companyYnDetail(A){
      ...
      this.$router.push({path:'companyDetail',query:{uuid: A}})
    },
         // 分页
    ...
    top5CompanyList(page,size){
        this.$http.get('/exhibitioncompanyapi/companyPageList?areaUuid='+this.uuid+'&page='+page +'&size='+size).then(res =>{
          this.normalCompanyList = res.data.data.content;
